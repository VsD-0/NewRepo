...
namespace ManagementDocument.API.Validations
{
    ...
    public class CreateDocumentValidator : AbstractValidator<CreateDocumentCommand>
    {
        ...
        /// <summary>
        /// Инициализирует новый экземпляр класса <see cref="CreateDocumentValidator"/>.
        /// </summary>
        ...
        public CreateDocumentValidator(IDocumentService documentService, IDocumentTypeService documentTypeService)
        {
        ...
        bool NumValidation(CreateDocumentCommand doc) => !_documentService.IsExist(doc.Num ?? throw new ArgumentNullException(nameof(doc.Num), "Номер документа является null"), doc.DocType);
        bool IsExistDocType(int type) => _documentTypeService.IsExistById(type);
        bool BirthDateValidation(CreateDocumentCommand doc)
        {
